...
DELETE FROM School
LEFT JOIN (SELECT Name, ParentName, Place_ID, COUNT(School_ID) as count
FROM School
GROUP BY Name, ParentName, Place_ID) as x
ON School.Name=x.Name AND School.ParentName=x.ParentName AND School.Place_ID=x.Place_ID
WHERE count>1